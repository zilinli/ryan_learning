...
This agent is remotely controlled via WeChat by its owner. Read `WEIXIN_COMMANDS.md` for the common command patterns (system status, Cursor coding, web search, cron reminders, file organization) and follow its response style (concise Chinese, no markdown tables in WeChat).
