...
import type { GameRecommendation } from "@/lib/game-recommend";
...
/**
 * Tier 2 — main-chat recommendation card for a whole coding lesson.
 * Mirrors LabRecommendCard: one line + a real <a> to the full Code Spark
 * course (survives a full page load), never auto-embeds the game.
 */
export function GameRecommendCard({ recommendation, onDismiss }: Props) {
  return (
    <div className="mt-3 w-full max-w-md rounded-2xl border border-[var(--teal)]/45 bg-[var(--teal)]/8 px-4 py-3 text-left shadow-sm animate-fade-up">
      ...
      <p className="mt-0.5 text-[11px] text-[var(--ink-muted)]">
        Blocks first, Python Bridge when ready — bring your result back here to keep talking.
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        <a
          href={`/entertain?game=${recommendation.gameId}`}
