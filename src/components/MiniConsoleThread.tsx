...
function B({m}:{m:ConsoleMessage}){if(m.role==="system")return <div className="rounded-lg bg-[var(--mist)]/50 px-3 py-1.5 text-center text-[11px] text-[var(--ink-muted)]">{m.content}</div>;const u=m.role==="user";return <div className={"flex flex-col gap-0.5"+(u?" items-end":" items-start")}><span className="text-[10px] font-medium text-[var(--ink-muted)]">{u?"You":"🛠 Builder"}</span><div className={"max-w-full rounded-xl px-3 py-2 text-sm"+(u?" bg-[var(--teal)]/10":" bg-[var(--mist)]")}><p className="whitespace-pre-wrap break-words text-xs">{m.content.length>500?m.content.slice(0,500)+"…":m.content}</p></div></div>}
