...
export function buildTutorPrompt(params: {
  ...
  const thinkFirstRules = [
    ...
    "",
    "Collaboration intents (optional hidden fence — the app parses it, never explain it to the student):",
    "When the student clearly wants to (a) write/draft/improve a piece of writing, (b) create a song/image/video, (c) relax with a game, (d) practice coding hands-on, or (e) watch/read something from our labs, you MAY emit AT MOST ONE hidden intent fence at the very end of your reply:",
    '~~~intent {"kind":"writing","text":"the draft or topic they want to write"} ~~~',
    '~~~intent {"kind":"media","text":"the creation idea they described"} ~~~',
    '~~~intent {"kind":"game","text":"topic hint for picking a relax game"} ~~~',
    '~~~intent {"kind":"lab","text":"topic they want to explore"} ~~~',
    "Coding / programming / CS (Scratch, loops, algorithms, variables, functions, computational thinking, 编程, 计算思维, 变量, 函数) uses a THREE-TIER rule — do NOT jump straight to a game:",
    "Tier 0 — a conceptual question (what is a variable/loop/function? how does it work?) is a NORMAL tutoring turn: answer plainly, ask a question back, and emit NO fence.",
    "Tier 1 — the student wants to TRY it hands-on (试试/练一练/做个小挑战/try it/practice): emit a micro-challenge keyed to the exact concept:",
    '~~~intent {"kind":"coding","concept":"loop"} ~~~',
    '~~~intent {"kind":"coding","concept":"conditional"} ~~~',
    '~~~intent {"kind":"coding","concept":"sequence"} ~~~',
    "Pick concept=loop for for/while/repeat/循环/重复; concept=conditional for if/condition/条件/判断/分支; otherwise concept=sequence.",
    "Tier 2 — the student explicitly wants a WHOLE lesson/level (玩一关/打开 Code Spark/上编程课/play a level):",
    '~~~intent {"kind":"coding","scope":"full"} ~~~',
    "Only emit a fence when a collaboration genuinely fits the request — never force one onto a normal tutoring turn. Keep the visible reply normal and complete by itself.",
    "",
