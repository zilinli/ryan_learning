...
describe("createConsoleHarnessTools", () => {
  ...
  it("returns the same tools instance", () => {
    expect(createConsoleHarnessTools()).toBe(tools);
  });

  describe("list_files", () => {
    it("lists directory contents", async () => {
      await fs.writeFile(path.join(TMP, "a.ts"), "a");
      await fs.writeFile(path.join(TMP, "b.ts"), "b");
      await fs.writeFile(path.join(TMP, ".hidden"), "h");
      const r = asString(await tools.list_files!.execute({ dirpath: "_test_tmp" }, {} as any));
  ...
  describe("read_file", () => {
    it("reads a file with line numbers", async () => {
      ...
      const r = asString(await tools.read_file!.execute({ filepath: "_test_tmp/x.ts" }, {} as any));
      expect(r).toContain("1|line1");
      expect(r).toContain("2|line2");
    });
  ...
  describe("edit_file", () => {
    it("replaces a string in a file", async () => {
      ...
      const r = asString(await tools.edit_file!.execute({ filepath: "_test_tmp/f.ts", old_string: "world", new_string: "earth" }, {} as any));
      expect(r).toContain("+0");
      const c = await fs.readFile(path.join(TMP, "f.ts"), "utf-8");
      expect(c).toBe("hello earth");
    });
  ...
  describe("git_diff", () => {
    it("returns a string for diff", async () => {
      const r = asString(await tools.git_diff!.execute({}, {} as any));
      expect(typeof r).toBe("string");
    });
  });

  describe("run_tests", () => {
    it("runs vitest and reports results", async () => {
      const r = asString(await tools.run_tests!.execute({ file: "src/lib/console-harness.test.ts" }, {} as any));
  ...
  describe("apply_changes", () => {
    it("rejects when tests fail", async () => {
      // Without any edits, run_tests will run the full suite (some may fail) or pass.
      // This test verifies the apply_changes flow throws on test failure.
      // Since we can't guarantee test state, we verify it runs and either throws or returns.
      try {
        await tools.apply_changes!.execute({ message: "test apply" }, {} as any);
      } catch (e) {
        // Expected if there are failing tests or no changes
  ...
  describe("revert_changes", () => {
    it("reverts uncommitted changes", async () => {
      const r = asString(await tools.revert_changes!.execute({}, {} as any));
      expect(typeof r).toBe("string");
    });
  });
});

describe("file change counter", () => {
  it("resets to zero", () => {
