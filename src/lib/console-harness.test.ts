...
describe("createConsoleHarnessTools", () => {
  ...
  describe("git_diff", () => { it("returns a string", async () => { const r = asString(await tools.git_diff!.execute({}, {} as any)); expect(typeof r).toBe("string"); }); });
  describe("run_tests", () => { it("runs vitest and reports results", async () => { const r = asString(await tools.run_tests!.execute({ file: "src/lib/console-harness.test.ts" }, {} as any)); const parsed = JSON.parse(r); expect(parsed.passed).toBeGreaterThanOrEqual(0); }, 70000); });
  ...
  describe("revert_changes", () => { it("reverts changes", async () => { const r = asString(await tools.revert_changes!.execute({}, {} as any)); expect(typeof r).toBe("string"); }); });
});
