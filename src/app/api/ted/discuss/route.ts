 ...
 * Body: { action: "open" | "reply", context, studentReply?, history? }
 */
...
function parseContext(raw: unknown): TedDiscussContext | null {
  ...
  return {
    talkTitle: String(c.talkTitle || "TED talk").slice(0, 200),
    speaker: String(c.speaker || "Speaker").slice(0, 120),
...
export async function POST(req: Request) {
  const limited = checkApiRateLimit(req, "ted-discuss", RATE_PRESETS.agent);
  ...
  const history: TedDiscussTurn[] = Array.isArray(body.history)
    ? body.history
        .map((t) => ({
  ...
  if (action === "open") {
    let reply = buildTedDiscussOpenerLocal(ctx);
    try {
      const full = await runAgent(discussOpenAgentPrompt(ctx), req.signal);
  ...
  let reply = buildTedDiscussReplyLocal(ctx, studentReply);
  try {
    const full = await runAgent(
      discussReplyAgentPrompt(ctx, history, studentReply),
      req.signal,
    );
    if (full.length > 12) reply = full;
  } catch {
