...
function evalPrompt(params: {
  ...
  return [
    "You are a warm, encouraging documentary coach. Evaluate the student's answer against the rubric. Do NOT summarize or rewrite — only judge.",
    "",
    gradeLine,
    `Clip: ${params.videoId}`,
...
export async function POST(req: Request) {
  const limited = checkApiRateLimit(req, "bbc-evaluate", RATE_PRESETS.agent);
  ...
  if (!studentAnswer) {
    return Response.json({
      ...
      feedback: "You haven't written an answer yet — even a guess helps you learn! Give it a try.",
      outcome: "incorrect" as TurnOutcome,
    });
  }

  const videoId = String(body.videoId || "clip").trim();
  ...
  try {
    ...
    if (!parsed) {
      ...
      return Response.json({
        ...
        feedback:
          v === "correct"
            ? "Good answer! Can you add one more detail to support your point?"
            : v === "partial"
              ? "Nice try! Try mentioning one specific scene or fact from the documentary."
              : "Give it another shot — what's one thing you remember from the clip?",
        fallback: true,
    ...
    if (err instanceof CursorAgentError) {
      return Response.json({
        ...
        feedback: "Nice effort! The coach is resting — but every answer helps you grow. Try another clip!",
        fallback: true,
        degraded: true,
