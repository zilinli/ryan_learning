...
```
smart-build.mjs
  ├── Phase 1: pre-clean (.next rm, gc if available)
  ├── Phase 2: memory check (warn if < 800 MB free)
  ├── Phase 3: next build with:
  │     NODE_OPTIONS="--max-old-space-size=1024"
  │     --no-mangling
  │     typescript.ignoreBuildErrors:true
  │     outputFileTracingExcludes (reduce NFT walk)
  ├── Phase 4: post-clean (.next/cache/webpack if exists)
  └── Graceful fallback:
        → retry at 768 MB
        → fallback to webpack (lower peak memory)
```

### Key Strategies

1. **Memory ceiling**: `NODE_OPTIONS="--max-old-space-size=1024"` caps Node heap at 1 GB
2. **Pre-clean `.next`**: Remove stale cache before every build, reclaiming 100+ MB
...
7. **Graceful fallback**: Retry with lower heap, then webpack, then selective build
