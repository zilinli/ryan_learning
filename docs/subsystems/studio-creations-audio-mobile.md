...
### Root cause
Studio generate persists audio via `writeMediaBytes` with `sessionId: "lyric-studio"`. Chat retention (`enforceServerRetention` → `pruneOrphanMedia`) only keeps chat session IDs + message attachment mediaIds. After the 2‑minute grace window, studio audio is treated as an orphan session and **deleted**, while `creations.json` still references the dead id.

Desktop and mobile both fail to decode; native mobile players make the failure more obvious.

### Approach
1. **Never prune** media whose `sessionId` is a reserved studio id (`lyric-studio`).
...
| Layer | Cases |
|-------|--------|
| Unit | prune keeps `lyric-studio` audio even when session not in keepIds |
| Unit | `deleteMediaForSession("lyric-studio")` is a no-op when protected |
